    ...
    class DJI_ROS_ControlPlugin : public ModelPlugin{
        public:
            ...
            void gpsCallback(const sensor_msgs::NavSatFix::ConstPtr& gps_msg)
            {
                ...
                ignition::math::Vector3d local_position = this->spherical_coordinates_handle->LocalFromSpherical(ignition::math::Vector3d(lat,lon,alt));
            ...
            void OnUpdate(){
                ...
                #if GAZEBO_MAJOR_VERSION >= 8
                    gimbal_pose.Set(this->gimbal_yaw_link->RelativePose().Pos(),this->gimbal_orientation);
                #else
                    ignition::math::Vector3d relative_position;
                    gazebo::math::Vector3 relative_position_gz = this->gimbal_yaw_link->GetRelativePose().pos;
                    relative_position.Set(relative_position_gz.x, relative_position_gz.y, relative_position_gz.z);
